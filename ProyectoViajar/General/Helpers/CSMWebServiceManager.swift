...
//  webService
//
//  Created by Kenyi Rodriguez and Mateo Espinoza on 4/03/18.
//  Copyright © 2018 Kenyi Rodriguez and Mateo Espinoza. All rights reserved.
//
...
private class CSMWebServiceFactorySessionTask{
    ...
    /**
     ...
     - Returns:
     */
    class func getURLSessionTask(url : String, method : CSMWebServiceMethod, data : Data?, session : URLSession, request : URLRequest, completionHandler: @escaping (_ webServiceResponse: CSMWebServiceResponse) -> Void)->URLSessionTask{
...
private class CSMWebServiceBodyRequest {
    ...
    init(contentType : CSMWebServiceContentType, parameters : [String:Any]?) {
        ...
        if let parametersToParse = self.parameters{
            ...
            for(key, value) in parametersToParse{
                ...
                if value is UIImage{
                    ...
                    let image_data = UIImagePNGRepresentation(value as! UIImage)
...
public class CSMWebServiceRequest{
    ...
    private func webServiceRequest(method : CSMWebServiceMethod,
        ...
        let task = CSMWebServiceFactorySessionTask.getURLSessionTask(url: urlString, method: method, data: data, session: session, request: request, completionHandler: completionHandler)
